...
test("同期エンジンを実サーバと突き合わせる", async () => {
  ...
  assert.equal(
    phone.vault.files.get(copies[0]),
    "# 細石刃\n\n電話の編集。\n",
    "自分の版が失われている",
  );
  ...
  const offlineCopies = [...phone.vault.files.keys()].filter((path) =>
    path.startsWith("offline (Conflicted copy"),
  );
  assert.equal(
    offlineCopies.length,
    1,
    "オフライン中の編集が競合コピーになっていない(上書きで消えた可能性)",
  );
  assert.equal(
    phone.vault.files.get(offlineCopies[0]),
    "# オフライン検証\n\n電話側で書いた大事な文章。\n",
    "オフライン中に書いた本文が失われている",
  );
  assert.equal(
    phone.vault.files.get("offline.md"),
    "# オフライン検証\n\nラップトップ側の続き。\n",
    "原本にリモート版が入っていない",
  );
  ...
  const madeHere = [...clone.vault.files.keys()].filter((path) =>
    path.includes("Conflicted copy clone"),
  );
