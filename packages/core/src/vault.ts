...
export class VaultDO extends DurableObject<Env> {
  ...
  private saltValue(): string {
    const [row] = [
      ...this.sql.exec<{ value: string }>("SELECT value FROM meta WHERE key = 'token_salt'"),
    ];
  ...
  async createDevice(name: string, scope: string, token: string): Promise<{ id: string }> {
    ...
    this.sql.exec(
      ...
      id,
      name,
      await tokenHash(this.saltValue(), token),
      scope,
      Date.now(),
    );
    return { id };
  ...
  async authenticate(token: string): Promise<{ id: string; scope: string } | null> {
    const find = (hash: string) =>
      [
        ...this.sql.exec<{ id: string; scope: string }>(
          "SELECT id, scope FROM devices WHERE token_hash = ? AND revoked_at IS NULL",
          hash,
        ),
      ][0] ?? null;
  ...
  async listDevices() {
    return [
      ...this.sql.exec(
        "SELECT id, name, scope, last_seen_seq, created_at FROM devices WHERE revoked_at IS NULL",
      ),
    ];
  }
  ...
  async readNote(path: string): Promise<{ rev: string; body: string } | null> {
    const [row] = [
      ...this.sql.exec<{ rev: string; body: string }>(
        `SELECT f.rev AS rev, n.body AS body FROM files f
       JOIN notes n ON n.path = f.path WHERE f.path = ? AND f.deleted = 0`,
        path,
      ),
    ];
    return row ?? null;
  ...
  private writeNote(path: string, body: string, wasIndexed: boolean, index?: NoteIndex): void {
    ...
    const [existing] = [
      ...this.sql.exec<{ id: number; title: string | null; body: string }>(
        "SELECT id, title, body FROM notes WHERE path = ?",
        path,
      ),
    ];
    ...
    if (existing) {
      ...
      this.sql.exec(
        "UPDATE notes SET title = ?, body = ?, frontmatter_json = ? WHERE id = ?",
        title,
        body,
        JSON.stringify(index?.frontmatter ?? null),
        existing.id,
      );
      id = existing.id;
    } else {
  ...
  private replaceIndex(path: string, index?: NoteIndex): void {
    ...
    for (const link of index?.links ?? []) {
      this.sql.exec(
        "INSERT INTO links(src_path, dst_path, kind) VALUES (?, ?, ?)",
        path,
        link.dst,
        link.kind,
      );
    ...
    for (const heading of index?.headings ?? []) {
      this.sql.exec(
        "INSERT INTO headings(path, level, text, line, parent_line) VALUES (?, ?, ?, ?, ?)",
        path,
        heading.level,
        heading.text,
        heading.line,
  ...
  async restore(path: string): Promise<PushResult> {
    const [row] = [
      ...this.sql.exec<{ body: string }>("SELECT body FROM notes WHERE path = ?", path),
    ];
  ...
  async structure(tool: string, args: Record<string, unknown>): Promise<unknown> {
    switch (tool) {
      ...
      case "note_outline":
        return [
          ...this.sql.exec(
            "SELECT level, text, line FROM headings WHERE path = ? ORDER BY line",
            args.path,
          ),
        ];
      case "backlinks":
        return [
          ...this.sql.exec(
           ...
           WHERE l.dst_path = ? ORDER BY l.src_path`,
            args.path,
          ),
        ];
      case "outlinks":
        return [
          ...this.sql.exec(
           ...
           WHERE l.src_path = ? ORDER BY l.dst_path`,
            args.path,
          ),
        ];
      ...
      case "hub_notes":
        return [
          ...this.sql.exec(
           ...
           ) GROUP BY path ORDER BY degree DESC, path LIMIT ?`,
      ...
      case "find_by_tag":
        return args.tag
          ? [
              ...this.sql.exec(
                "SELECT DISTINCT path FROM tags WHERE tag = ? ORDER BY path",
                args.tag,
              ),
            ]
          : [
  ...
  private tree(depth: number, prefix: string) {
    const rows = [
      ...this.sql.exec<{ path: string }>(
        "SELECT path FROM files WHERE deleted = 0 AND path LIKE ? ORDER BY path",
        `${prefix}%`,
      ),
    ];
  ...
  private neighborhood(path: string, hops: number) {
    ...
    const nodes = [
      ...this.sql.exec<{ path: string; hop: number }>(
       ...
       SELECT path, min(hop) AS hop FROM reachable GROUP BY path ORDER BY hop, path`,
        path,
        Math.max(0, Math.min(hops, 5)),
      ),
    ];
    const within = new Set(nodes.map((node) => node.path));
    const edges = [
      ...this.sql.exec<{ src_path: string; dst_path: string; kind: string }>(
  ...
  private readSection(path: string, heading: string) {
    ...
    const headings = [
      ...this.sql.exec<{ level: number; text: string; line: number }>(
        "SELECT level, text, line FROM headings WHERE path = ? ORDER BY line",
        path,
      ),
    ];
    ...
    if (index === -1) {
      throw new Error(
        `no heading "${heading}" in ${path}. available: ${headings.map((h) => h.text).join(", ")}`,
      );
    }
  ...
  async status() {
    const [row] = [
      ...this.sql.exec<{ n: number }>("SELECT count(*) AS n FROM files WHERE deleted = 0"),
    ];
    return {
      files: row.n,
