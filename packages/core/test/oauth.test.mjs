...
test("OAuth の認可経路を通す", async () => {
  ...
  const resourceMetadata = await fetch(`${base}/.well-known/oauth-protected-resource`).then((r) =>
    r.json(),
  );
  assert.ok(resourceMetadata.authorization_servers?.length > 0, "authorization_servers が無い");

  const asMetadata = await fetch(`${base}/.well-known/oauth-authorization-server`).then((r) =>
    r.json(),
  );
  for (const field of [
    "issuer",
  ...
  const register = (clientName) =>
    fetch(asMetadata.registration_endpoint, {
      method: "POST",
  ...
  const exchange = (body) =>
    fetch(asMetadata.token_endpoint, {
      method: "POST",
      headers: { "content-type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams(body),
    });

  const redeem = (code, verifier, clientId = client.client_id) =>
    exchange({
  ...
  assert.ok(
    (await consent.text()).includes("Test MCP Client"),
    "同意画面がクライアント名を出していない",
  );

  // 誤った秘密では発行されない
  assert.equal(
    (await obtainCode({ secret: "wrong" })).status,
    401,
  ...
  const firstUse = await redeem(reused.code, reused.verifier, replayClient.client_id).then((r) =>
    r.json(),
  );
  ...
  const afterReplay = await fetch(`${base}/mcp`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${firstUse.access_token}`,
  ...
  const rpc = (method, params = {}, token = tokens.access_token) =>
    fetch(`${base}/mcp`, {
      method: "POST",
      headers: {
  ...
  assert.ok(
    listed.body.result?.tools?.some((tool) => tool.name === "vault_tree"),
  ...
  assert.deepEqual(
    tree.body.result.structuredContent.rootNotes,
    ["bound.md"],
    "トークンが同意時の Vault に束縛されていない",
  );

  assert.equal(
    (await rpc("tools/list", {}, "not-a-real-token")).status,
    401,
    "でたらめなトークンが通ってしまう",
  );
  ...
  for (let attempt = 0; attempt < 20 && !mine; attempt++) {
    const listing = await fetch(`${base}/grants`, {
  ...
  for (let attempt = 0; attempt < 4 && !listedVault; attempt++) {
    const vaultList = await fetch(`${base}/vaults`, {
      headers: { Authorization: `Bearer ${admin}` },
    }).then((r) => r.json());
